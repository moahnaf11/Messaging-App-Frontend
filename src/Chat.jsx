...
export const FriendNotificationContext = createContext();
...
function Chat() {
  ...
  const [friendNoti, setFriendNoti] = useState(false);
  ...
  useEffect(() => {
    setFriendNoti(pendingRequestsList.length > 0);
  ...
  const handleAdminToggle = async (groupId) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/admin-only`,
        {
          method: "PUT",
          headers: {
  ...
  const updateGroupChatDisplay = async (groupId, boolvalue) => {
    try {
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/archive`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateChatDisplay = async (chatId, action) => {
    try {
      ...
      setOpenMenu(null);
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/friend/request/archive/${chatId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const removeMember = async (groupId, userId) => {
    try {
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/remove-member`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateGroupMemberRole = async (groupId, memberId, newRole) => {
    try {
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/role`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const addMember = async (e, groupId) => {
    ...
    try {
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/add-member`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e, groupId) => {
    ...
    try {
      const response = await fetch(
        `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/upload-photo`,
        {
          method: "PUT",
    ...
    socket.on("receiveFriendNotification", async (notification) => {
      ...
      if (pathname.current !== `/chat/${id}`) {
        ...
        try {
          ...
          const response = await fetch(
            `https://messaging-app-backend-p1g9.onrender.com/friend/${id}/noti`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
      ...
      async ({ senderId, groupId, notification }) => {
        if (pathname.current === `/group/${groupId}`) {
          try {
            ...
            // Fetch the notifications for this group from the backend (localhost:3000)
            const response = await fetch(
              `https://messaging-app-backend-p1g9.onrender.com/group/noti/delete`,
              {
                method: "DELETE",
                headers: {
                  "Content-Type": "application/json",
          ...
          try {
            ...
            // Fetch the notifications for this group from the backend (localhost:3000)
            const response = await fetch(
              `https://messaging-app-backend-p1g9.onrender.com/group/${groupId}/group-notis`,
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
    ...
    async function fetchData() {
      try {
        ...
        const [friendsRes, groupChatsRes] = await Promise.all([
          fetch(`https://messaging-app-backend-p1g9.onrender.com/friend`, {
            method: "GET",
            signal,
            headers: {
          ...
          fetch(`https://messaging-app-backend-p1g9.onrender.com/group`, {
            method: "GET",
            signal,
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    >
      ...
      >
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? "p-3 font-custom font-bold bg-gray-700 border-l-4 border-blue-600 flex items-center gap-4 md:text-[12px] lg:text-[16px] justify-between"
              : "p-3 font-custom font-bold bg-gray-800 border-l-4 border-gray-800 flex items-center gap-4 hover:bg-gray-700 md:text-[12px] lg:text-[16px] justify-between"
          }
        >
          ...
          >
            <UpdateChatDisplayContext.Provider value={{ updateChatDisplay }}>
              <FriendNotificationContext.Provider
                value={{ friendNoti, setFriendNoti, pendingRequestsList }}
              >
                <Outlet
                  context={{
                    friends,
